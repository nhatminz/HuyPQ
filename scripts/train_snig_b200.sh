...
export RUN_NAME="${RUN_NAME:-${SNIG_RUN_NAME:-snig_$(date +%Y%m%d_%H%M%S_%N)}}"
...
if [[ -n "${RESUME_FROM_CHECKPOINT}" && "${RESUME_FROM_CHECKPOINT}" != "auto" && -z "${OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${OUTPUT_DIR:-${SNIG_RUN_OUTPUT}}"
fi
build_training_args "${OUTPUT_DIR}"
echo "SNIG run: ${RUN_NAME}"
echo "SNIG output: ${OUTPUT_DIR}"
...
run_training_cli train \
  ...
  "${COMMON_TRAIN_ARGS[@]}" "$@"
if [[ -n "${RESUME_FROM_CHECKPOINT}" ]]; then
  echo "SNIG resume completed in: ${OUTPUT_DIR}"
else
  echo "SNIG completed. Keep this identifier: SNIG_RUN_NAME=${RUN_NAME}"
fi
