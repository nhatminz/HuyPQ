...
resolve_run_paths
MODEL_PATH="${SNIG_CHECKPOINT:-${SNIG_RUN_OUTPUT}/final}"
EVAL_OUTPUT="${SNIG_EVAL_OUTPUT:-${RUN_RESULTS_DIR}/eval/snig_opd}"
require_file "${MODEL_PATH}/config.json"
cd "${REPO_DIR}"
exec "${PYTHON_BIN}" -m b200_experiment.cli evaluate \
  --config "${SNIG_CONFIG}" --name "SNIG-OPD" --model "${MODEL_PATH}" \
  "${ASSET_CONFIG_ARGS[@]}" \
  --output "${EVAL_OUTPUT}" \
