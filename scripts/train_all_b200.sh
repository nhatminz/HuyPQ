...
# Environment values supplied when launching the script still t ake precedence.
...
export VLLM_GPU_WORKSPACE_HEADROOM_GIB="${VLLM_GPU_WORKSPACE_HEADROOM_GIB:-2}"
...
if [[ "${RUN_PGT_TRAIN}" == "true" ]]; then
  RUN_NAME="${PGT_RUN_NAME}" OUTPUT_DIR="${PGT_RUN_OUTPUT}" \
    STUDENT_MODEL="${STUDENT_MODEL}" TEACHER_MODEL="${TEACHER_MODEL}" \
    TRAIN_DATA="${TRAIN_DATA}" PROMPT_KEY="${PROMPT_KEY}" \
    RESUME_FROM_CHECKPOINT="${PGT_RESUME_FROM_CHECKPOINT:-}" \
    bash "${SCRIPT_DIR}/train_pgt_b200.sh"
fi
...
if [[ "${RUN_IW_TRAIN}" == "true" ]]; then
  RUN_NAME="${IW_RUN_NAME}" OUTPUT_DIR="${IW_RUN_OUTPUT}" \
    ...
    MICRO_BATCH_SIZE_PER_GPU="${IW_MICRO_BATCH_SIZE_PER_GPU:-8}" \
    ROLLOUT_VLLM_GPU_MEMORY_UTILIZATION="${IW_ROLLOUT_VLLM_GPU_MEMORY_UTILIZATION:-0.60}" \
