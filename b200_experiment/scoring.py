...
class BaseScores:
    ...
    # Joint student/teacher full-vocabulary quantities.  They are populated
    # only by ``score_student_teacher_rollout`` because computing them requires
    # both logit tensors to coexist.  Keeping only [batch, time] reductions
    # avoids retaining either vocabulary-sized tensor after scoring.
    full_log_ratio_mean: torch.Tensor | None = None
    full_log_ratio_variance: torch.Tensor | None = None
    full_common_mass: torch.Tensor | None = None
    cache: object | None = None


def _joint_full_vocab_metrics(
    ...
    *,
    ...
    """Reduce exact full-vocabulary PGT and common mass in bounded chunks.

    For ``r = log q - log p`` this returns ``E_p[r]``, ``Var_p[r]``, and
    ``sum min(p, q)``.  The last quantity is the one-step maximal-coupling
    agreement probability.  All arithmetic is FP32, while the two model-logit
    views remain in their inference dtype.
    """
    if student_logits.shape != teacher_logits.shape:
        raise ValueError("Student and teacher logits must share one shape")
    means, variances, common_masses = [], [], []
    ...
    for begin in range(0, width, chunk_steps):
        ...
        student = student_logits[:, begin:end].float() / float(student_temperature)
        teacher = teacher_logits[:, begin:end].float() / float(teacher_temperature)
        log_p = student - student_log_normalizers[:, begin:end].unsqueeze(-1)
        log_q = teacher - teacher_log_normalizers[:, begin:end].unsqueeze(-1)
        p = log_p.exp()
        r = log_q - log_p
        mean = (p * r).sum(dim=-1)
        second_moment = (p * r.square()).sum(dim=-1)
        means.append(mean)
        # Roundoff can make E[r^2]-E[r]^2 very slightly negative.
        variances.append((second_moment - mean.square()).clamp_min(0.0))
        common_masses.append(torch.minimum(log_p, log_q).exp().sum(dim=-1))
    return (
        torch.cat(means, dim=1),
        torch.cat(variances, dim=1),
        torch.cat(common_masses, dim=1),
    )
...
def score_student_teacher_rollout(
    ...
    *,
    ...
    trim_padding: bool = True,
    length_bucketed: bool = True,
    compute_full_vocab_metrics: bool = False,
    ...
    to three independent ``score_original_rollout`` calls.  Full-vocabulary
    reductions are optional diagnostics; support-matched CMT does not require
    them in the training hot path.
    """
    ...
    for batch_begin in range(0, batch_size, score_micro_batch):
        ...
        if compute_full_vocab_metrics:
            (
                student_local.full_log_ratio_mean,
                student_local.full_log_ratio_variance,
                student_local.full_common_mass,
            ) = _joint_full_vocab_metrics(
                student_logits,
                teacher_logits,
                student_local.scaled_log_normalizers,
                teacher_local.scaled_log_normalizers,
                score_chunk_steps=score_chunk_steps,
    ...
    def finish(batches: dict[str, list[torch.Tensor]]) -> BaseScores:
        ...
        return BaseScores(
            ...
            full_log_ratio_mean=restored("full_log_ratio_mean"),
            full_log_ratio_variance=restored("full_log_ratio_variance"),
            full_common_mass=restored("full_common_mass"),
        )
